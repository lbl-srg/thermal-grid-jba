...
model DetailedPlantFiveHubsWithRequirementsVerification
 ...
 "Valves actuator values for the control valves of each ETS (heat exchanger, condenser loop of the chiller, evaporator loop of the chiller, domestichot water when present)";
  ...
  Buildings_Requirements.GreaterEqual reqPDis[nBui + 1](
  ...
  Modelica.Blocks.Sources.RealExpression PDis[nBui + 1](y(each unit="Pa/m")=fracPL)
    "Pressure drop in the district loop"
    annotation (Placement(transformation(extent={{580,-540},{600,-520}})));
  Modelica.Blocks.Sources.Constant fracPLMax[nBui + 1](each k(each unit="Pa/m")=125)
...
equation
  ...
  connect(fracPLMax.y,reqPDis. u_max) annotation (Line(points={{601,-490},{610,-490},
          {610,-504},{619,-504}}, color={0,0,127}));
  connect(PDis.y,reqPDis. u_min) annotation (Line(points={{601,-530},{610,-530},
          {610,-508},{619,-508}}, color={0,0,127}));
  connect(THeaDiff.y,reqTHea. u) annotation (Line(points={{601,280},{610,280},{610,
          274},{619,274}},        color={0,0,127}));
  connect(TCooDiff.y,reqTCoo. u) annotation (Line(points={{601,240},{610,240},{610,
  ...
  connect(TRooHea.y,TRooHeaDif. u2) annotation (Line(points={{441,-160},{448,-160},
  ...
  connect(THexPriLvg.y,reqTHexEtsPriLvg. u) annotation (Line(points={{601,110},{
          610,110},{610,114},{619,114}},
                                  color={0,0,127}));
  connect(TRooHeaAvg60min.y,reqTRooHea. u_min) annotation (Line(points={{602,-130},
          {610,-130},{610,-128},{619,-128}}, color={0,0,127}));
  connect(TRooHeaDifMax.y,reqTRooHea. u_max) annotation (Line(points={{601,-100},
          {610,-100},{610,-124},{619,-124}}, color={0,0,127}));
  connect(TRooHeaDifYea.y,reqTRooHeaAvg. u_max) annotation (Line(points={{601,-160},
          {610,-160},{610,-172},{619,-172}}, color={0,0,127}));
  ...
  connect(TRooCooDif.y,TRooCooAvg60min. u) annotation (Line(points={{481,-320},{
          560,-320},{560,-310},{578,-310}}, color={0,0,127}));
  connect(TRooCooAvg60min.y,reqTRooCoo. u_min) annotation (Line(points={{602,-310},
          {610,-310},{610,-308},{619,-308}}, color={0,0,127}));
  connect(TRooCooDifMax.y,reqTRooCoo. u_max) annotation (Line(points={{601,-280},
          {610,-280},{610,-304},{619,-304}}, color={0,0,127}));
  connect(TRooCooDifYea.y,reqTRooCooAvg. u_max) annotation (Line(points={{601,-340},
          {610,-340},{610,-352},{619,-352}}, color={0,0,127}));
