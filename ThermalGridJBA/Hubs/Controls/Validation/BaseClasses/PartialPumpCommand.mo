within ThermalGridJBA.Hubs.Controls.Validation.BaseClasses;
model PartialPumpCommand "Validation model for the pump control blocks"
  ...
  Buildings.Fluid.FixedResistances.PressureDrop preDroSec(
    redeclare final package Medium = Medium,
    final m_flow_nominal=mSec_flow_nominal,
    dp_nominal=40E3) "Pressure drop of secondary loop"         annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-50,-40})));
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter heaToMas(final k=1/
        QLoa_flow_nominal*mSec_flow_nominal)
  ...
  Buildings.Fluid.Sources.Boundary_pT bou(redeclare final package Medium =
    ...
    annotation (Placement(transformation(
        ...
        rotation=180,
        origin={210,20})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senTRet(redeclare final package
      Medium = Medium, m_flow_nominal=mSec_flow_nominal,
    T_start=T_start)
