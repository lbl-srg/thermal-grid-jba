...
model AirToWater "Validation of AWHP plant template"
  ...
  inner parameter
    ...
    datAll(pla(final cfg=pla.cfg,
               hp(final capHeaHp_nominal=capHp_nominal,
                  final capCooHp_nominal=capHp_nominal)))
                                   "Plant parameters"
  ...
  parameter String filNamLoa = "modelica://ThermalGridJBA/Resources/Data/Hubs/Individual/1380.mos"
    "File name for building load";
  parameter Modelica.Units.SI.HeatFlowRate capHp_nominal =
    max(abs(QCoo_flow_nominal), abs(QHea_flow_nominal))/pla.nHp
    "Capacity of each HP";
  parameter Modelica.Units.SI.HeatFlowRate QCoo_flow_nominal(
    max=-Modelica.Constants.eps)=
      Buildings.DHC.Loads.BaseClasses.getPeakLoad(
        ...
        filNam=Modelica.Utilities.Files.loadResource(filNamLoa))
  ...
  Modelica.Blocks.Sources.CombiTimeTable loa(
    tableOnFile=true,
    tableName="tab1",
    fileName=Modelica.Utilities.Files.loadResource(filNamLoa),
  ...
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter norFlo[if have_chiWat
     ...
     k=if have_chiWat then {1/pla.mHeaWat_flow_nominal,1/pla.mChiWat_flow_nominal}
         else {1/pla.mHeaWat_flow_nominal}) "Normalize flow rate"
                           annotation (
  ...
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter norLoa[2](each k=1/
        capHp_nominal) "Normalize load against capacity" annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-90,80})));
equation
  ...
  connect(ph[2].y, mulInt[4].u2) annotation (Line(points={{18,124},{12,124},{12,
          114},{2,114}}, color={255,127,0}));
  connect(norLoa.y, ctlEquZon.u_s)
    annotation (Line(points={{-78,80},{68,80}}, color={0,0,127}));
  connect(loa.y[2:3], norLoa[1:2].u)
    annotation (Line(points={{-159,80},{-102,80}}, color={0,0,127}));
  annotation (
    __Dymola_Commands(
      file=
