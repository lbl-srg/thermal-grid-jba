...
response = requests.get(f"{base_url}/projects", headers=headers, data=payload)
...
response = requests.get(f"{base_url}/projects/{projectGuid}", headers=headers, data=payload)
...
scenario_new = scenario_name.replace('single', 'multi')
...
for k in rp:
    ...
    if any(k in name for k in hub_current.keys()):
        payload = {
            ...
            "hubGuids": [hub_current[j] for j in hub_current.keys() if j in name],
            "name": name,
            "reverse": False,
            "stages": [stage0],
            "demandSalePrice":0
...
#%% Upload on-site resources

response = requests.get(f"{base_url}/v2/scenarios/{scenarioGuid}/solar-on-site-resource", headers=headers)
rp = json.loads(response.text)['data']

lect = pd.read_excel(additional_values, sheet_name='pv', index_col=0)

for k in rp:
    resource_name = k['name']
    l_hub = []
    for j in lect.index:
        hub_dict = {"hubGuid": hub_current[j], 
                    "availableResourceType":"Area", 
                    "availableSolarCollectorArea": int(lect.loc[j, resource_name])}
        l_hub.append(hub_dict)        
    
    payload = {
        "energyCarrierGuid": k['energyCarrierGuid'],
        "hubs": l_hub,
        "name": resource_name,
        "profileId":k['profileId'],
        "stages":[stage0],
        "solarResourceName":resource_name,
        "profile": "UPLOADED"
    }
    response = requests.put(f"{base_url}/v2/scenarios/solar-on-site-resource/{k['solarResourceGuid']}", json=payload, headers=headers)
...
if 'plant' in hub_current.keys(): 
    ...
    networkTechnologyGuid = json.loads(response.text)['data']['networkTechnologies'][0]['networkTechnologyGuid']
    ...
    for k in network.index:
        payload = {"mustBeInstalled": False,
                    ...
                    "networkTechnologyGuid": networkTechnologyGuid,
                    "costComponents": [],
                    "name": network.loc[k, 'name']}
